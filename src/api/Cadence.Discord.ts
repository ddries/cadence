...
import CadenceSpotify from "./Cadence.Spotify";
...
export default class CadenceDiscord {
    ...
    private async OnInteraction(interaction: discord.Interaction): Promise<void> {
        ...
        if (interaction.isCommand()) {
            let command = interaction.commandName.toLocaleLowerCase();

            try {
                this._commands.get(command)?.run(interaction);
            } catch (e) {
                this.logger.log('could not execute command ' + command + ' in ' + this.resolveGuildNameAndId(interaction.guild) + ': ' + e);
            } finally {
            ...
            if (interaction.targetMessage.embeds.length > 0) {
                ...
                if (CadenceLavalink.getInstance().isValidUrl(url)) {
                    if (url.includes("youtube") || url.includes("you")) {
                        result = await CadenceLavalink.getInstance().resolveLinkIntoTracks(url);
                    } else {
                        if (url.includes("playlist")) {
                            result = await CadenceSpotify.getInstance().resolveLinkIntoSpotifyPlaylist(url);
                        } else {
                            result = await CadenceSpotify.getInstance().resolveLinkIntoTracks(url);
                        }
                    }
                } else {
                    result = await CadenceLavalink.getInstance().resolveYoutubeIntoTracks(url.trim());
                }
